...
核心组件:
- TopkDropoutStrategy: 基于预测排序的动量淘汰策略 (from src.strategies)
- EqualWeightStrategy: 等权配置策略 (from src.strategies)
- ScoreWeightStrategy: 预测得分加权策略 (from src.strategies)
- PortfolioSimulator: 回测模拟器 (含交易成本)
- RiskManager: 风控模块 (最大回撤/止损)
...
class PortfolioSimulator:
    ...
    def run(
        ...
        if self.positions:
            ...
            self._liquidate(final_prices, final_date)
    ...
    def _liquidate(self, prices: pd.Series, date: str):
        """清仓所有持仓"""
        for inst in list(self.positions.keys()):
            price = prices.get(inst, 0) if len(prices) > 0 else 0
            if price > 0:
                self._sell_position(inst, price, date)
